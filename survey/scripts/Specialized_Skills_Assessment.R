...
colnames(df)
df |>
  select(Spec_design,Spec_data,Spec_networking,Spec_digitalMarketing,
...
# Plot Parameters ---------------------------------------------------------
glue("Specialized Skills Assessment") |> 
  str_to_title() -> title
...
ggsave(filename = glue("Specialized_Skills_Assessment_CUTSAMPLE.pdf"),
       path = file.path(here("survey","results","magazine")),
       width=13,height= 6,units="in",dpi=1000)

# 
# 
# ggsave(filename = glue("Specialized_Skills_Assessment_CUTSAMPLE.png"),
