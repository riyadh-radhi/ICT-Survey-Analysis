...
colnames(df)
unique(df$Basic_workplace)
df |>
  select(Basic_selfStudy,Basic_college,
         Basic_workplace,Basic_trainings,
         Prog_selfStudy,Prog_college,
         Prog_workplace,Prog_trainings,
         Spec_selfStudy,Spec_college,
         Spec_workplace,Spec_trainings) |> 
  pivot_longer(cols = contains(c("Basic", "Spec", "Prog")),
               names_to = "feature",values_to = "evaluation")  |> 
  mutate(feature = str_split(feature,pattern = "_",simplify = T)[,2]) -> long_df 
...
theme(
  ...
  plot.title = element_markdown(size = 16,hjust =0.5,vjust = -0.7,color = ka_col$title["1"]),
  ...
  plot.caption = element_markdown(color = ka_col$title["1"],face = "italic",
                                  size = 14,hjust = 0,padding = unit(c(5, 0, 0, 0), "pt")),
  
  #####################                     AXIS                                  ################
  #axis.title.x = element_text(color = ka_col$title["1"],size = 12),
  axis.title.x =element_text(color = ka_col$title["1"],
                             size=15),
...
# Plot Parameters ---------------------------------------------------------
glue("Ratings of Different Knowledge Sources in Acquiring Digital Skills Among Iraqis") |> 
  str_to_title() -> title

#subtitle <- glue(" <span style = 'color: {ka_col$turquaz[['10']]}'>Within The Private Sector In General</span>")
subtitle <- ""
#caption <- "The presented results were collected on Septemper 2021 with a total sample size of 540 responders."
caption <-glue("<br></br><span style = 'color: {ka_col$source['1']}'>Note:</span>
                  The presented results were collected on September 2021 with a total sample size of 540 responders.")
...
ggsave(filename = glue("combined_knowledge_source.pdf"),
