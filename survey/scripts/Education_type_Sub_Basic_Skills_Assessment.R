...
df |>
  filter(Education_type != "Unkown") |> 
  select(Education_type,Basic_email,Basic_searching,Basic_vedioConf,Basic_word,
  ...
  mutate(total = case_when(Education_type %in% "Public"  ~ sub_group_count$Public,
                           Education_type %in% "Private"  ~ sub_group_count$Private)) |> 
...
ggsave(filename = glue("Education_type_Sub_group_Basic_Skills_Assessment.png"),
