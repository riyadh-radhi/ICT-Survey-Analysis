...
df |> 
  filter(digit_related %in% c("Specialized & Programming Digital Skills",
                              "Programming Digital Skills")) -> df


df |> 
  group_by(Education_type) |> 
  count() |> 
  pivot_wider(names_from = Education_type,values_from = n) -> sub_group_count

colnames(df)
df |>
  filter(!Education_type %in% c("Unkown")) |> 
  select(Education_type,Prog_webDev,Prog_Desktop,Prog_mobileDev,Prog_DS,
         Prog_DB,Prog_stats,Prog_versionMang) |> 
  pivot_longer(cols = contains("Prog"),
               names_to = "feature",values_to = "evaluation") |> 
  mutate(feature = str_sub(string = feature,start = 6)) |> 
  group_by(Education_type,feature) |> 
  add_count(evaluation) |> 
  distinct(Education_type, feature,evaluation,n) |> 
  arrange(Education_type, feature,evaluation) |> 
  mutate(total = case_when(Education_type %in% "Private"  ~ sub_group_count$Private,
                           Education_type %in% "Public"  ~ sub_group_count$Public)) |> 
  mutate(perc = (n/total)*100,
         evaluation = factor(evaluation)) |> 
  mutate(evaluation = fct_relevel(evaluation,"Very Poor",
                                  "Poor","Average",
                                  "Good","Excellent"),
         feature = case_when(feature %in% "webDev" ~ "Web Development",
                             feature %in% "Desktop" ~ "Desktop Application Development ",
                             feature %in% "mobileDev" ~ "Mobile Application Development",
                             feature %in% "DS" ~ "Data Science",
                             feature %in% "DB" ~ "Database Management",
                             feature %in% "stats" ~ "Statistical Programming",
                             feature %in% "versionMang" ~ "Version Control Tools")) |>
...
# Plot Parameters ---------------------------------------------------------
glue("SubGroup Programming Skills Assessment") |> 
  str_to_title() -> title
...
for(i in unique(df_bar$feature)){
  df_bar |> 
    filter(feature %in% i) |> 
    ggplot(aes(x=Education_type,y= perc, fill = evaluation))+
    ...
    theme_bw()+
    riyadh_theme+
    geom_text(aes(x=Education_type,
                  y= perc,
...
(plots$`Web Development` + plots$`Mobile Application Development` +
    plots$`Data Science` )  / (plots$`Database Management` +
                                 plots$`Version Control Tools` +
                                 plots$`Statistical Programming`+
                                 plots$`Desktop Application Development `)

ggsave(filename = glue("Education_type_Sub_group_programming_Skills_Assessment.png"),
