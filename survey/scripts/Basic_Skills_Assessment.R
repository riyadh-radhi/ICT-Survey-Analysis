...
#to install ggchicklet package
#install.packages("ggchicklet", repos = "https://cinc.rud.is")

df <- read_xlsx(here("survey","clean_data","ICT_clean.xlsx"))
colnames(df)
df |>
  select(Basic_email,Basic_searching,Basic_vedioConf,Basic_word,
  ...
  mutate(evaluation = fct_relevel(evaluation,"Very Poor",
                                  "Poor","Average",
                                  "Good","Excellent"),
         feature = case_when(feature %in% "email" ~ "Email Communication",
...
ggsave(filename = glue("Basic_Skills_Assessment.pdf"),
       path = file.path(here("survey","results","magazine")),
       width=13,height= 6,units="in",dpi=1000)

# ggsave(filename = glue("Basic_Skills_Assessment.png"),
